...
class BackgroundCest
{
    ...
    public function imageAdapterImagickBackground(UnitTester $I)
    {
        ...
        $image
            ->background('#000')
            ->save(outputDir('image/imagick/background.jpg'))
        ;

        $I->amInPath(outputDir('image/imagick/'));
        $I->seeFileFound('background.jpg');
        $I->assertGreaterThan(200, $image->getWidth());
        $I->assertGreaterThan(200, $image->getHeight());
